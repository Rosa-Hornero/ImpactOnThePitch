...
def render_arsenal_tab():
    ...
    with cola:
        st.image("static/images/escudos/arsenal_logo.png", caption="Logo of Arsenal", width=150)

    # Columna derecha: Historia del equipo
    with colb:
        st.write("""
        **Arsenal Football Club**  
        Established in 1886, Arsenal is one of the most successful and historically significant football clubs in England. Known for their attractive, attacking style of play and their iconic red and white jerseys, Arsenal plays their home matches at the Emirates Stadium in North London. The club boasts a rich tradition of success both domestically and internationally, having won numerous league titles and cups. Arsenal is renowned for its focus on developing talented players and its commitment to maintaining a strong, competitive presence in English and European football.
         """)
    ...
    # Datos Arsenal
    arsenal_data = data[data['Team Name'] == 'Arsenal']


    # Filtrar por temporada
    seasons = arsenal_data['Season'].unique()  
    selected_season = st.selectbox("Select a Season", options=seasons)
    fil_seas_data = arsenal_data[arsenal_data['Season'] == selected_season]
    ...
    if selected_player == None:
        ...
        st.write(f"Here you can see the general injury metrics of **Arsenal for the session {selected_season}**. You can see the **total number of injured players**, the **average injury time** and the **average age** of the injured players. You can also view different graphs to see the **most frequent type of injury** and the **most affected positions**. Also included is a **calendar** where you can see **all the injuries** of the team this season.")
