...
def render_aston_villa_tab():
    st.title('🦁 Aston Villa')
    ...
    with cola:
        st.image("static/images/escudos/aston_villa_logo.png", caption="Aston Villal", width=150)

    # Columna derecha: Historia del equipo
    with colb:
        st.write("""
            **Aston Villa Football Club**  
            Established in 1874, Aston Villa is one of the oldest and most historic football clubs in England. Based in Birmingham, the club plays its home matches at the iconic Villa Park. Known for their claret and blue jerseys, Aston Villa has a proud tradition of success, including multiple league titles and a European Cup triumph in 1982. The club is celebrated for its passionate fan base and its commitment to developing talent while striving for excellence in both domestic and international competitions.
        """)
    ...
    # Datos Aston Villa
    aston_data = data[data['Team Name'] == 'Aston Villa']
    #st.dataframe(aston_data)

    # Filtrar por temporada
    seasons = aston_data['Season'].unique()  
    selected_season = st.selectbox("Select a Season", options=seasons)
    fil_seas_data = aston_data[aston_data['Season'] == selected_season]
    ...
    if selected_player == None:
        ...
        st.write(f"Here you can see the general injury metrics of **Aston Villa for the session {selected_season}**. You can see the **total number of injured players**, the **average injury time** and the **average age** of the injured players. You can also view different graphs to see the **most frequent type of injury** and the **most affected positions**. Also included is a **calendar** where you can see **all the injuries** of the team this season.")
