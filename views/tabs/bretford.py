...
def render_bretford_tab():
    ...
    with cola:
        st.image("static/images/escudos/brentford_logo.png", caption="Logo of Brentford", width=150)

    # Columna derecha: Historia del equipo
    with colb:
        st.write("""
                **Brentford Football Club**  
                Established in 1889, Brentford is a club on the rise, known for its innovative approach and commitment to modern football analytics. Based in West London, the team plays its home matches at the Brentford Community Stadium. Nicknamed "The Bees," Brentford is celebrated for its attacking football and its dedication to developing young talent. The club's recent success in reaching and competing in the Premier League has earned it admiration across English football, showcasing a blend of tradition and forward-thinking in the modern game.
            """)
    ...
    # Datos Brentford
    brentford_data = data[data['Team Name'] == 'Brentford']


    # Filtrar por temporada
    seasons = brentford_data['Season'].unique()  
    selected_season = st.selectbox("Select a Season", options=seasons)
    fil_seas_data = brentford_data[brentford_data['Season'] == selected_season]
    ...
    if selected_player == None:
        ...
        st.write(f"Here you can see the general injury metrics of **Brentford for the session {selected_season}**. You can see the **total number of injured players**, the **average injury time** and the **average age** of the injured players. You can also view different graphs to see the **most frequent type of injury** and the **most affected positions**. Also included is a **calendar** where you can see **all the injuries** of the team this season.")
        ...
        elif posicion == 'Defensive Midfielder ':
            posicion='Defensive Midfielder'
