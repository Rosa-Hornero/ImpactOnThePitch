...
def render_tottenham_tab():
    st.title('𓅥 Tottenham')
    ...
    with col1:
        st.image("static/images/escudos/tottenham_logo.png", caption="Logo of Tottenham Hotspur", width=150)

    # Columna derecha: Historia del equipo
    with col2:
        st.write("""
            **Tottenham Hotspur Football Club**  
            Established in 1882, Tottenham Hotspur, commonly known as Spurs, is a club with a rich history and a
            passionate fan base. Known for their attacking football and iconic Lilywhite jerseys, Spurs play their home 
            matches at the state-of-the-art Tottenham Hotspur Stadium in North London. The club has a proud tradition 
            of producing world-class players and competing at the highest levels of English and European football.
        """)
    ...
    # Datos Newcastle
    tottenham_data = data[data['Team Name'] == 'Tottenham']
    
    # Filtrar por temporada
    seasons = tottenham_data['Season'].unique()  
    selected_season = st.selectbox("Select a Season", options=seasons)
    fil_seas_data = tottenham_data[tottenham_data['Season'] == selected_season]
    ...
    if selected_player == None:
        ...
        st.write(f"Here you can see the general injury metrics of **Tottenham for the session {selected_season}**. You can see the **total number of injured players**, the **average injury time** and the **average age** of the injured players. You can also view different graphs to see the **most frequent type of injury** and the **most affected positions**. Also included is a **calendar** where you can see **all the injuries** of the team this season.")
